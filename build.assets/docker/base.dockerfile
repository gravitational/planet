...
RUN curl -L https://github.com/coredns/coredns/releases/download/v$COREDNS_VER/release.coredns_$COREDNS_VER_linux_amd64.tgz -o /tmp/coredns-$COREDNS_VER.tar.gz && \
    mkdir -p /tmp/coredns && tar -xvzf /tmp/release.coredns_$COREDNS_VER_linux_amd64.tgz -C /tmp/coredns && \
