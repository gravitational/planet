...
// ElectionConfig represents configuration for the master election process
...
// String returns string representation of the agent election config
func (conf ElectionConfig) String() string {
	return fmt.Sprintf("ElectionConfig(key=%v, ip=%v, role=%v, term=%v, endpoints=%v, apiserverDNS=%v)",
		conf.LeaderKey, conf.PublicIP, conf.Role, conf.Term, conf.EtcdEndpoints, conf.APIServerDNS)
}

// startElection starts the master election loop and sets up callbacks
...
func startElection(conf *ElectionConfig) (io.Closer, error) {
...
func runAgent(conf *agent.Config, monitoringConf *monitoring.Config, electionConf *ElectionConfig, peers []string) error {
	if conf.Tags == nil {
	...
	client, err := startElection(electionConf)
	if err != nil {
		return trace.Wrap(err)
	}
	defer client.Close()
